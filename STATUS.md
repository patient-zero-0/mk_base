...
M2 数学分析  █████████████░░░░░░░  🚧 进行中（Ch3 + Ch4 已合并 main）
                                    Ch3 微分 25 条 ✅ · Ch4 积分 29 条 ✅ · Ch5 级数 待启动
                                    新增 54 条 draft（PR #19–#27 全部合并）
```
...
| M2 | 数学分析 Ch3–Ch5 | 第 6–9 周 | ≥ 80 条 stable，例题配套完成 | 🚧 进行中：Ch3（25）+ Ch4（29）已合并 main，Ch5（27）待启动；54 条 draft，待统一升 stable |
...
- Batch 10–13：数项级数 → 判别法 → 函数项 / 一致收敛 → 幂级数 / Taylor 级数（详见 `_meta/m2-plan.md`）

### M2 待办

- [ ] Ch5 级数 27 条 draft
- [ ] M2 全部 draft（54 条 + Ch5 27 条）统一评审升 `review → stable`
- [ ] 重新生成 `_meta/dependency-graph.json`（当前为 M1 快照 52 节点 / 201 边，未含 M2 条目）
...
| 首屏加载时间（TTI） | < 2s | 待测量 |
| 条目录入时间 | ≤ 30 min/条 | 待测量 |
| CI 检查通过率 | 100% | 4/4（GitHub Actions 已激活，PR #19–#27 全绿） |
| 悬空引用数 | 0（main 分支） | 0（全库 106 条目） |
| KaTeX 公式总量 | — | 6495 条，全部通过校验 |
| 条目状态分布 | review ≥ 80% 时启动 stable 评审 | stable = 47 / draft = 59（M2 新增 54 + algebra 4 占位 + cross 1） |

---

*M0 / M1 已全面闭环。M2 进行中：Ch3 微分（25）+ Ch4 积分（29）已合并 main，共 54 条 draft。*
*下一步：M2 Ch5 级数（27 条，Batch 10–13）— 详见 `_meta/m2-plan.md`。*
