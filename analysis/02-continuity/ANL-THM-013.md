...
- ✗ 误用反向："若 $\eta = f(\xi)$ 对某 $\xi \in (a, b)$ 成立，则 $\eta$ 必在 $f(a), f(b)$ 之间"。
  反例：$f(x) = x(x - 1)(x - 2)$ 在 $[0, 2]$ 上 $f(0) = f(2) = 0$，但 $f(1.6) = 1.6 \cdot 0.6 \cdot (-0.4) = -0.384$，
  $\xi = 1.6$ 处取得的函数值 $-0.384$ 显然**不在** $f(0) = 0$ 与 $f(2) = 0$ 之间（两端值相等时，"介于其间"的范围退化为空集）。
  本定理给出**单向**蕴含——值域可以远超端点值的范围。
